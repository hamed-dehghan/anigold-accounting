...
import { productModel, DocumentInfo } from '../../types';
...
import { FormField } from '../../../../components/ViewDocuments';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '../../../../components/ui/select';
import { convertImageUrl } from '../../../../lib/convertImageUrl';

interface ViewProdoctProps {
  rowInfo: productModel;
  onClose: () => void;
...
const ViewProdoct: React.FC<ViewProdoctProps> = ({
  rowInfo,
  onClose,
  onEditSuccess,
  onDeleteImage,
  ...
  const {
    ...
    defaultValues: {
      weight: rowInfo.weight,
      name: rowInfo.name,
      code: rowInfo.code,
      carat: rowInfo.carat,
      wages: rowInfo.wages,
      wageTypeId: rowInfo.wageTypeId,
      productCollection: rowInfo.productCollection, // Ensure default value is set
      quiddityProduct: rowInfo.quiddityProduct, // Ensure default value is set
    },
  });

  //Add this to watch weghit field
    ...
    onDrop: async (acceptedFiles) => {
      ...
      for (const file of acceptedFiles) {
        try {
          ...
          //   onAddImage(response.data.data.fileId, rowInfo.id);
          // }
        } catch (error: any) {
  ...
  const onSubmit = async (data: DocumentInfo) => {
    console.log('Submission Data:', data); // Debugging: Log the form data

    const newInfoProduct: any = {
      ...data,
      id: rowInfo.id,
      productCategoryId: rowInfo?.productCategoryId,
      wageTypeId: rowInfo?.wageTypeId,
      productCollection: Number(data.productCollection), // Ensure number type
      quiddityProduct: Number(data.quiddityProduct), // Ensure number type
    ...
    try {
      const response = await UpdateProducts(newInfoProduct);
      if (response.data.isSuccessful) {
        onEditSuccess(newInfoProduct);
        response.data.getMessageText.map((message: string) => {
          Toast({ message, type: 'success' });
        });
        onClose();
  ...
  const title = `مشاهدهِ محصولِ دسته بندیِ "${CategoryName}" `
  return (
    <Viewer
      ...
      headerTitlePrefix={title}
      infoTooltipContent="اینجا میتوانید جزییات محصول را مشاهده کنید"
      isEditMode={IsEdit}
      onEdit={handleEdit}
      onClose={onClose}
      classes={{
    ...
    >
      <div className='px-2 '>
        <div className="w-full border mt-5 md:mt-0"></div>
        {/* Dropzone and uploaded files section */}
        <div

          className={`${IsEdit === 'edit' && 'border-dashed border'} flex max-w-[90%] md:max-w-full    gap-4 mt-2 border p-3 rounded-[4px] border-boderFileUploader border-dashed h-[150px] cursor-pointer`}
        >
          {/* Scroll area for uploaded files */}
          <ScrollArea dir="rtl" className='max-w-xs lg:max-w-3xl'>
            <div className="flex gap-2">
              {/* Existing Images   */}
              {rowInfo?.productImages?.map((item: any, index: any) => (
                <div key={index} className="relative ">
                  ...
                  {IsEdit === 'edit' && < Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      onDeleteImage(item.fileCode, rowInfo.id);
                    }}
                  >
          ...
          {IsEdit === 'edit' && (
            <div
              ...
              <span className="text-[10px] text-white px-4 mb-2 leading-5 rounded-sm w-[calc(100%-10px)] text-center bg-blue_5">+ بارگزاری تصویر</span>
            </div>
          )}
        </div>
        ...
        {/* Form */}
        <form onSubmit={handleSubmit(onSubmit)} className="gap-5 my-3 text-gray_45 text-[14px]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 mb-[20px] *:mb-5">
            <FormField trigger={trigger} name="code" label="کد محصول" errors={errors} disabled="disabled" control={control} />

            <FormField trigger={trigger} name="name" control={control} label="نام محصول" disabled={IsEdit === 'view'} errors={errors} requrid={true} />
            <FormField trigger={trigger} name="weight" control={control} label="وزن" disabled={IsEdit === 'view'} errors={errors} />

            <FormField trigger={trigger} name="carat" control={control} label="عیار" disabled={IsEdit === 'view'} errors={errors} />
            {
              weightValue && <>
                <div className={`transition-all duration-300 ${weightValue ? 'opacity-100' : 'hidden'}`}>
                  <FormField
                    trigger={trigger}
                    name="percentageWage"
                    control={control}
            ...
            <div>
              <div className={`max-w-[90%] md:max-w-full border-2 border-[#023338] focus-within:border-[#9a732e] flex items-center justify-center ${errors.productCollection && 'border-red_5'} rounded-[5px]`}>
                <label className={`w-[180px] h-[37px] flex justify-center items-center rounded-[5px] ${errors.productCollection ? 'bg-errorInput' : 'bg-gray_5'}`}>
                  نوع محصول
                </label>
                <Controller
                  name="productCollection"
                  control={control}
                  defaultValue={rowInfo.productCollection} // Ensure default value
                  render={({ field }) => (
                    <Select
                      ...
                      disabled={IsEdit === 'view'}
                    >
                      <SelectTrigger className="border-t-0 border-b-0  border-l-0  border-r-2 border-[inherit] rounded-none rounded-l-[5px] rounded-br-[18px] shadow-none focus:ring-0 bg-white mr-[-12px]">
                        <SelectValue placeholder="نوع محصول">
                          {productCollection.find((collection) => collection.value === field.value)?.label || 'نوع محصول'}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
            ...
            <div>
              <div className={`max-w-[90%] md:max-w-full border-2 border-[#023338] focus-within:border-[#9a732e] flex items-center justify-center ${errors.productCollection && 'border-red_5'} rounded-[5px]`}>
                <label className={`w-[180px] h-[37px] flex justify-center items-center rounded-[5px] ${errors.quiddityProduct ? 'bg-errorInput' : 'bg-gray_5'}`}>
                  نوع
                </label>
                <Controller
                  name="quiddityProduct"
                  control={control}
                  defaultValue={rowInfo.quiddityProduct} // Ensure default value
                  render={({ field }) => (
                    <Select
                      onValueChange={(value) => field.onChange(value ? parseInt(value) : undefined)}
                      value={field.value?.toString() || ""} // Convert to string for Select
                    ...
                    >
                      <SelectTrigger className="border-t-0 border-b-0  border-l-0  border-r-2 border-[inherit] rounded-none rounded-l-[5px] rounded-br-[18px] shadow-none focus:ring-0 bg-white mr-[-12px]">
                        <SelectValue placeholder="نوع">
                          {quiddityProductCategory.find((quiddity) => quiddity.value === field.value)?.label || 'نوع'}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
          ...
          <div className="justify-center lg:justify-end flex">
            {IsEdit === 'edit' ? (
              <Button type="submit" variant="outline" className="w-[80px] h-[37px] bg-green_1 text-white">
                ذخیره
              </Button>
            ) : (
              <Button variant="outline" className="w-[129px] h-[37px] bg-gray_40 text-puple_1" onClick={onClose}>
                بستن
              </Button>
            )}
          </div>
