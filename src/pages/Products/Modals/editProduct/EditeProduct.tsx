...
import { DocumentInfo, productModel } from '../../types';
...
interface EditProductProp {
    ...
    onClose: () => void;
    onEditSuccess: (updatedProduct: productModel) => void; // Callback to update the local state
...
const EditeProduct: React.FC<EditProductProp> = ({ rowInfo, onClose, onEditSuccess, onDeleteImage, onAddImage, WageTypes, CategoryName }) => {

    const [files, setFiles] = useState<File[]>([]); // State to store uploaded files
    const uploadedFileRefs = useRef<{ [key: string]: string }>({}); // Ref to store file upload responses



    const { control, handleSubmit, reset, trigger, formState: { errors } } = useForm<DocumentInfo>({
        ...
        defaultValues: {
            ...
            quiddityProduct: rowInfo.quiddityProductCategoryId,
            productCollection: rowInfo.productCollectionId
        },
    });
    ...
    const { getRootProps, getInputProps } = useDropzone({
        ...
        onDrop: async (acceptedFiles) => {
            setFiles([...files, ...acceptedFiles]); // Add new files to state

            for (const file of acceptedFiles) {
                try {
                    const formData = new FormData();
                    formData.append("Content", file); // Fix: Append the actual file, not just the name

                    const response = await UploadSingleFile(formData);
                    console.log('response.data.data.fileId', response.data.data.fileId);
                    uploadedFileRefs.current[file.name] = response.data.data.fileId; // Store response
    ...
    console.log('ref', uploadedFileRefs.current);
    // Function to remove a file
    const removeFile = (file: File) => {
        setFiles(files.filter((f) => f !== file)); // Remove the file from the state
        delete uploadedFileRefs.current[file.name]; // Remove the file response from the ref
    };
    ...
    const onSubmit = async (data: DocumentInfo) => {
        ...
        try {
            ...
            if (response.data.isSuccessful) {
                ...
                // onAddImage(uploadedFileRefs.current, rowInfo.id);
    ...
    const title = `ویرایش محصولِ دسته بندیِ "${CategoryName}"`
    ...
    return (
        <Viewer
            ...
            infoTooltipContent="اینجا میتوانید جزییات محصول را ویرایش کنید"
            onClose={onClose}
            classes={{
        ...
        >
            <div>
                ...
                <div
                    ...
                    className="flex max-w-[90%] md:max-w-full  gap-4 mt-2 border p-3 rounded-[4px] border-boderFileUploader border-dashed h-[150px] cursor-pointer"
                >
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="gap-5 my-3 text-gray_45 text-[14px]">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 mb-[20px] *:mb-5">
                        ...
                        <div>
                            <div className={`max-w-[90%] md:max-w-full border-2 border-[#023338] focus-within:border-[#9a732e] flex items-center justify-center ${errors.productCollection && 'border-red_5'} rounded-[5px]`}>
                                <label className={`w-[180px] h-[37px] flex justify-center items-center rounded-[5px] ${errors.productCollection ? 'bg-errorInput' : 'bg-gray_5'}`}>
                                    نوع محصول
                                    <span className="text-red_5">*</span>
                                </label>
                                <Controller
                        ...
                        <div>
                            <div className={`max-w-[90%] md:max-w-full border-2 border-[#023338] focus-within:border-[#9a732e] flex items-center justify-center ${errors.productCollection && 'border-red_5'} rounded-[5px]`}>
                                <label className={`w-[180px] h-[37px] flex rounded-[5px] justify-center items-center  ${errors.quiddityProduct ? 'bg-errorInput' : 'bg-gray_5'}`}>
                                    نوع
                                    <span className="text-red_5">*</span>
                                </label>
                                <Controller
