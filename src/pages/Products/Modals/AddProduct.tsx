...
const AddProducts: React.FC<AddProductsProps> = ({ categoryID, onClose, onAddSuccess, CategoryName }) => {
    const { control, handleSubmit, reset, trigger, formState: { errors } } = useForm<any>();
        ...
        try {
            const bodyApi = {
                ...
                productImages: Object.values(uploadedFileRefs.current).map((fileCode) => ({
                    fileCodes: fileCode,
                    isMain: 'false'
                })), // Use the stored file names/codes
                isMain: null,
            };

            const response = await AddProduct(bodyApi); // Call the AddProduct API
            if (response.data.isSuccessful) {
                onAddSuccess(bodyApi); // Update the local state
                onClose();
                reset();
    ...
    const title = `ایجاد محصولِ دسته بندیِ "${CategoryName}"`
    return (
        <Viewer
        ...
        >
            <div className=' max-w-xs  md:max-w-full lg:w-full  mx-auto md:mx-0'>
                ...
                >
                    <div className='flex  justify-between items-start gap-2 order-3 md:order-1'>
                        <span className='truncate'>{title}</span>
                        <TooltipProvider content={'می توانید محصول خود را اضافه کنید'} isShowTooltip={false}>
                            <Icon
                                icon="question"
                ...
                <div className="flex max-w-[90%] md:max-w-full  gap-4 mt-2 border p-3 rounded-[4px] border-boderFileUploader border-dashed h-[150px] cursor-pointer"
                >
                ...
                <form onSubmit={handleSubmit(onSubmit)}
                    className="gap-5 my-3 text-gray_45 text-[14px] ">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 mb-[20px]   *:mb-5">
                        ...
                        <FormField
                            ...
                            label="عیار"
                            // rules={{
                            //     required: 'وارد کردن عیار اجباری است',
                            //     valueAsNumber: true,
                            //     min: { value: 500, message: 'عیار باید بین 500 تا 999.9 باشد' },
                            //     max: { value: 999.9, message: 'عیار باید بین 500 تا 999.9 باشد' }
                            // }}
                            errors={errors}
                        />
                            ...
                            weightValue && <>
                                <div className={`transition-all duration-300 ${weightValue ? 'opacity-100' : 'hidden'}`}>
                                    <FormField
                                        ...
                                        rules={{
                                            valueAsNumber: true,
                                            min: { value: 0, message: 'عیار باید بین 0 تا 100 باشد' },
                                            max: { value: 100, message: 'عیار باید بین 0 تا 100 باشد' }
                                        }}
                                        type="tel"
                                        errors={errors}
                                    />
                        ...
                        <div>
                            <div className={`border-2 max-w-[90%] md:max-w-full border-[#023338] ${errors.quiddityProduct && '!border-red_5'} focus-within:border-[#9a732e] flex items-center justify-center  rounded-[5px]`}>
                                <label className={`w-[160px] h-[37px] flex justify-center items-center rounded-[5px] rounded-l-none ${errors.productCollection ? 'bg-errorInput' : 'bg-gray_5'}`}>
                                    نوع
                                </label>
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        ...
                                        >
                                            <SelectTrigger
                                                ...
                                                className="border-t-0 border-b-0 border-l-0 border-r-2 border-[inherit] rounded-none rounded-l-[5px] rounded-br-[18px] shadow-none focus:ring-0 bg-white mr-[-12px]"
                                            >
                                                <SelectValue >
                        ...
                        <div>
                            <div className={`max-w-[90%] md:max-w-full border-2 border-[#023338] focus-within:border-[#9a732e] flex items-center justify-center ${errors.productCollection && '!border-red_5'} rounded-[5px]`}>
                                <label className={`w-[160px] px-3 h-[37px] flex justify-center items-center rounded-[5px] ${errors.productCollection ? 'bg-errorInput' : 'bg-gray_5'}`}>
                                    اکسسوری
                                </label>
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        ...
                                        >
                                            <SelectTrigger
                                                ...
                                                className="border-t-0 border-b-0 border-l-0 border-r-2 rounded-none border-[inherit] rounded-l-[5px] rounded-br-[18px] shadow-none focus:ring-0 bg-white mr-[-12px]"
                                            >
                                                <SelectValue >
                    ...
                    <div className="justify-center lg:justify-end flex">
                        <Button
                            ...
                            type="submit"
                            variant="outline"
                            className="h-[37px] text-white hover:text-white text-[14px] text-center bg-green_1 hover:bg-green-600"
                        >
                            ذخیره
                        </Button>
                    </div>
