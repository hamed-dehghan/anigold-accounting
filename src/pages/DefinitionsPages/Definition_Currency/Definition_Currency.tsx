...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Imports ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
import './Definition_Currency.scss';
...
import Currency_Form from '../../DefinitionForms/Currency_Form/Currency_Form';
import { Messages } from '../../../utils/messages';




function Definition_CurrencyType() {
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ State Variables ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  const [isDialogOpen, setDialogOpen] = useState(false);
  const [rerender, setRerender] = useState(false);
  const [Loading, setLoading] = useState(false);
  const [CurrencyType_List, setCurrencyType_List] :any = useState([]);
  const [SelectCurrencyType, setSelectCurrencyType] : any= useState({});
  ...
  function handleOpen(object :any) {
    if(object){
      ...
      setSelectCurrencyType(object?.Row_Selected)
    }
    setDialogOpen(true);
  };
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Render Functions ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  function Get_allCurrencyType(){
    ...
    setLoading(true)
    apiService.Get_allCurrencyType(generateQuery(QueryParams))
    .then((response:any) => {
        setLoading(false)
        ...
        setCurrencyType_List(response.data.data.data)
    })
    .catch((error:any) => {
      
    });
  }
  function Get_CurrencyTypeBuId(Id:any){
    // apiService.Get_CurrencyTypeGetById(Id)
    // .then((response:any) => {
    //   console.log(response.data.data);
    //   setSelectCurrencyType(...SelectCurrencyType,response.data.data)
    // })
    // .catch((error:any) => {
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Config Table ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  ...
  const ConfirmationCallback = (childData:any) => {
    ...
    apiService.Delete_CurrencyType(data)
    .then((response:any) => {
      if(response?.data?.isSuccessful){
        setOpenNotification({open:true,type:'success',object:{messageType:'delete',name:'ارز',nameValue:childData?.row?.name,code:childData?.row?.code}})
        Get_allCurrencyType()
      }else{
        ...
        <Group gap={4} justify="right" wrap="nowrap">
          ...
          >
            <ConfirmationDialog 
              ...
              Theme="danger"
              Body= {Messages?.deleteMessage_modal({name:'ارز',nameValue:customer?.name,code:customer?.code})}
              Title="حذف ارز"
          />
          </ActionIcon>
          <ActionIcon
          ...
          >
             <Tooltip title="ویرایش ارز" componentsProps={{ tooltip: { className: 'tooltip-warning' } }}>
  ...
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Main Return (JSX) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  return (
    <div className="Definition_Currency page-container" >
      <div className="page-container-header">
        <div className='page-container-header-toolbar'>
          <div className='page-header-title'>تعریف ارز</div>
      ...
      <CustomDialog
          isOpen={isDialogOpen}
          onClose={handleClose}
          modalTitle={formType=='Edit' ? "ویرایش ارز" : "تعریف ارز" }
          OptionalComponent={Currency_Form}
          onSave={handleSave}
          handleModalClick={() => console.log('Modal clicked')}
          index={1000}
          componentProps = {{selectRow:SelectCurrencyType,formType:formType}}
          minHeight = '10rem'
          minWidth = '15rem'
      />
    </div>
...
export default Definition_CurrencyType;
