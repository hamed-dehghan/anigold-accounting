...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Imports ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
import './Definition_Incometype.scss';
...
import Incometype_Form from '../../DefinitionForms/Incometype_Form/Incometype_Form';



function Definition_Incometype() {
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ State Variables ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  const [isDialogOpen, setDialogOpen] = useState(false);
  const [rerender, setRerender] = useState(false);
  const [Loading, setLoading] = useState(false);
  const [Incometypes_List, setIncometypes_List] :any = useState([]);
  const [SelectIncometypes, setSelectIncometypes] : any= useState({});
  ...
  function handleOpen(object :any) {
    if(object){
      ...
      setSelectIncometypes(object?.Row_Selected)
    }
    setDialogOpen(true);
  };
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Render Functions ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  function Get_allIncometypes(){
    ...
    setLoading(true)
    apiService.Get_allIncometype(generateQuery(QueryParams))
    .then((response:any) => {
        setLoading(false)
        ...
        setIncometypes_List(response.data.data.data)
    })
    .catch((error:any) => {
      
    });
  }
  function Get_IncometypesBuId(Id:any){
    // apiService.Get_IncometypesGetById(Id)
    // .then((response:any) => {
    //   console.log(response.data.data);
    //   setSelectIncometypes(...SelectIncometypes,response.data.data)
    // })
    // .catch((error:any) => {
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Config Table ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  ...
  const ConfirmationCallback = (childData:any) => {
    ...
    apiService.Delete_Incometype(data)
    .then((response:any) => {
      if(response?.data?.isSuccessful){
        setOpenNotification({open:true,type:'success',object:{messageType:'delete',name:'نوع درآمد',nameValue:childData?.row?.name,code:childData?.row?.code}})
        Get_allIncometypes()
      }else{
        ...
        <Group gap={4} justify="right" wrap="nowrap">
          ...
          >
            <ConfirmationDialog 
              id={customer?.id}
              row={customer}
              tooltip="حذف نوع درآمد"
              parentCallback={(childData: any) => { ConfirmationCallback(childData); }}
              ButtonText="DeleteIcon"
              OkButtonText="حذف"
              CancelButtonText="انصراف"
              Theme="danger"
              Body= {Messages?.deleteMessage_modal({name:'نوع درآمد',nameValue:customer?.name,code:customer?.code})}
              Title="حذف نوع درآمد"
          />
          </ActionIcon>
          <ActionIcon
          ...
          >
             <Tooltip title="ویرایش نوع درآمد" componentsProps={{ tooltip: { className: 'tooltip-warning' } }}>
  ...
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Main Return (JSX) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  return (
    <div className="Definition_Incometype page-container" >
      <div className="page-container-header">
        <div className='page-container-header-toolbar'>
          <div className='page-header-title'>تعریف نوع درآمد</div>
      ...
      <CustomDialog
          isOpen={isDialogOpen}
          onClose={handleClose}
          modalTitle={formType=='Edit' ? "ویرایش نوع درآمد" : "تعریف نوع درآمد" }
          OptionalComponent={Incometype_Form}
          onSave={handleSave}
          handleModalClick={() => console.log('Modal clicked')}
          index={1000}
          componentProps = {{selectRow:SelectIncometypes,formType:formType}}
          minHeight = '10rem'
          minWidth = '15rem'
      />
    </div>
...
export default Definition_Incometype;
