...
import Cost_Form from '../../DefinitionForms/Cost_Form/Cost_Form';
import { Messages } from '../../../utils/messages';



function Definition_Cost() {
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ State Variables ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  const [isDialogOpen, setDialogOpen] = useState(false);
  const [rerender, setRerender] = useState(false);
  const [Loading, setLoading] = useState(false);
  const [Cost_List, setCost_List] :any = useState([]);
  const [SelectCost, setSelectCost] : any= useState({});
  ...
  function handleOpen(object :any) {
    if(object){
      ...
      setSelectCost(object?.Row_Selected)
    }
    setDialogOpen(true);
  };
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Render Functions ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  function Get_allCost(){
    ...
    setLoading(true)
    apiService.Get_allCost(generateQuery(QueryParams))
    .then((response:any) => {
        setLoading(false)
        ...
        setCost_List(response.data.data.data)
    })
    .catch((error:any) => {
      
    });
  }
  function Get_CostBuId(Id:any){
    // apiService.Get_CostGetById(Id)
    // .then((response:any) => {
    //   console.log(response.data.data);
    //   setSelectCost(...SelectCost,response.data.data)
    // })
    // .catch((error:any) => {
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Config Table ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  ...
  const ConfirmationCallback = (childData:any) => {
    ...
    apiService.Delete_Cost(data)
    .then((response:any) => {
      if(response?.data?.isSuccessful){
        setOpenNotification({open:true,type:'success',object:{messageType:'delete',name:'هزینه',nameValue:childData?.row?.name,code:childData?.row?.code}})
        Get_allCost()
      }else{
        ...
        <Group gap={4} justify="right" wrap="nowrap">
          ...
          >
            <ConfirmationDialog 
              ...
              Theme="danger"
              Body= {Messages?.deleteMessage_modal({name:'هزینه',nameValue:customer?.name,code:customer?.code})}
              Title="حذف هزینه"
          />
          </ActionIcon>
          <ActionIcon
          ...
          >
             <Tooltip title="ویرایش هزینه" componentsProps={{ tooltip: { className: 'tooltip-warning' } }}>
  ...
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Main Return (JSX) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  return (
    <div className="Definition_Cost page-container" >
      <div className="page-container-header">
        <div className='page-container-header-toolbar'>
          <div className='page-header-title'>تعریف هزینه </div>
      ...
      <CustomDialog
          isOpen={isDialogOpen}
          onClose={handleClose}
          modalTitle={formType=='Edit' ? "ویرایش هزینه" : "تعریف هزینه" }
          OptionalComponent={Cost_Form}
          onSave={handleSave}
          handleModalClick={() => console.log('Modal clicked')}
          index={1000}
          componentProps = {{selectRow:SelectCost,formType:formType}}
          minHeight = '10rem'
          minWidth = '15rem'
      />
    </div>
...
export default Definition_Cost;
