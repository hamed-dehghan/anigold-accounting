...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Imports ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
import './Definition_Costtype.scss';
...
import Costtype_Form from '../../DefinitionForms/Costtype_Form/Costtype_Form';



function Definition_Costtype() {
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ State Variables ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  const [isDialogOpen, setDialogOpen] = useState(false);
  const [rerender, setRerender] = useState(false);
  const [Loading, setLoading] = useState(false);
  const [Costtypes_List, setCosttypes_List] :any = useState([]);
  const [SelectCosttypes, setSelectCosttypes] : any= useState({});
  ...
  function handleOpen(object :any) {
    if(object){
      ...
      setSelectCosttypes(object?.Row_Selected)
    }
    setDialogOpen(true);
  };
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Render Functions ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  function Get_allCosttypes(){
    ...
    setLoading(true)
    apiService.Get_allCosttypes(generateQuery(QueryParams))
    .then((response:any) => {
        setLoading(false)
        ...
        setCosttypes_List(response.data.data.data)
    })
    .catch((error:any) => {
      
    });
  }
  function Get_CosttypesBuId(Id:any){
    // apiService.Get_CosttypesGetById(Id)
    // .then((response:any) => {
    //   console.log(response.data.data);
    //   setSelectCosttypes(...SelectCosttypes,response.data.data)
    // })
    // .catch((error:any) => {
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Config Table ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  ...
  const ConfirmationCallback = (childData:any) => {
    ...
    apiService.Delete_Costtypes(data)
    .then((response:any) => {
      if(response?.data?.isSuccessful){
        setOpenNotification({open:true,type:'success',object:{messageType:'delete',name:'نوع هزینه',nameValue:childData?.row?.name,code:childData?.row?.code}})
        Get_allCosttypes()
      }else{
        ...
        <Group gap={4} justify="right" wrap="nowrap">
          ...
          >
            <ConfirmationDialog 
              id={customer?.id}
              row={customer}
              tooltip="حذف نوع هزینه"
              parentCallback={(childData: any) => { ConfirmationCallback(childData); }}
              ButtonText="DeleteIcon"
              OkButtonText="حذف"
              CancelButtonText="انصراف"
              Theme="danger"
              Body= {Messages?.deleteMessage_modal({name:'نوع هزینه',nameValue:customer?.name,code:customer?.code})}
              Title="حذف نوع هزینه"
          />
          </ActionIcon>
          <ActionIcon
          ...
          >
             <Tooltip title="ویرایش نوع هزینه" componentsProps={{ tooltip: { className: 'tooltip-warning' } }}>
  ...
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Main Return (JSX) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  return (
    <div className="Definition_Costtype page-container" >
      <div className="page-container-header">
        <div className='page-container-header-toolbar'>
          <div className='page-header-title'>تعریف نوع هزینه</div>
      ...
      <CustomDialog
          isOpen={isDialogOpen}
          onClose={handleClose}
          modalTitle={formType=='Edit' ? "ویرایش نوع هزینه" : "تعریف نوع هزینه" }
          OptionalComponent={Costtype_Form}
          onSave={handleSave}
          handleModalClick={() => console.log('Modal clicked')}
          index={1000}
          componentProps = {{selectRow:SelectCosttypes,formType:formType}}
          minHeight = '10rem'
          minWidth = '15rem'
      />
    </div>
...
export default Definition_Costtype;
