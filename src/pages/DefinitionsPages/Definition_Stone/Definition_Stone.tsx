...
import Stone_Form from '../../DefinitionForms/Stone_Form/Stone_Form';
import { Messages } from '../../../utils/messages';




function Definition_Stone() {
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ State Variables ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  const [isDialogOpen, setDialogOpen] = useState(false);
  const [rerender, setRerender] = useState(false);
  const [Loading, setLoading] = useState(false);
  const [StoneType_List, setStoneType_List] :any = useState([]);
  const [SelectStoneType, setSelectStoneType] : any= useState({});
  ...
  function handleOpen(object :any) {
    if(object){
      ...
      setSelectStoneType(object?.Row_Selected)
    }
    setDialogOpen(true);
  };
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Render Functions ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  function Get_allStoneType(){
    ...
    setLoading(true)
    apiService.Get_allStoneType(generateQuery(QueryParams))
    .then((response:any) => {
        setLoading(false)
        ...
        setStoneType_List(response.data.data.data)
    })
    .catch((error:any) => {
      
    });
  }
  function Get_StoneTypeBuId(Id:any){
    // apiService.Get_StoneTypeGetById(Id)
    // .then((response:any) => {
    //   console.log(response.data.data);
    //   setSelectStoneType(...SelectStoneType,response.data.data)
    // })
    // .catch((error:any) => {
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Config Table ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  ...
  const ConfirmationCallback = (childData:any) => {
    ...
    apiService.Delete_StoneType(data)
    .then((response:any) => {
      if(response?.data?.isSuccessful){
        setOpenNotification({open:true,type:'success',object:{messageType:'delete',name:'سنگ',nameValue:childData?.row?.persianName,code:childData?.row?.code}})
  ...
  const columns :any= [
    ...
    { title:'نام فارسی' , accessor: 'persianName' , width: 200 ,sortable: true, ...props}, 
    { title:'نام انگلیسی' , accessor: 'englishName' , width: 200 ,sortable: true, ...props}, 
        ...
        <Group gap={4} justify="right" wrap="nowrap">
          ...
          >
            <ConfirmationDialog 
              ...
              Theme="danger"
              Body= {Messages?.deleteMessage_modal({name:'سنگ',nameValue:customer?.persianName,code:customer?.code})}
              Title="حذف سنگ"
          />
          </ActionIcon>
          <ActionIcon
          ...
          >
             <Tooltip title="ویرایش سنگ" componentsProps={{ tooltip: { className: 'tooltip-warning' } }}>
  ...
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Main Return (JSX) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  return (
    <div className="Definition_Stone page-container" >
      <div className="page-container-header">
        <div className='page-container-header-toolbar'>
          <div className='page-header-title'>تعریف سنگ</div>
      ...
      <CustomDialog
          isOpen={isDialogOpen}
          onClose={handleClose}
          modalTitle={formType=='Edit' ? "ویرایش سنگ" : "تعریف سنگ" }
          OptionalComponent={Stone_Form}
          onSave={handleSave}
          handleModalClick={() => console.log('Modal clicked')}
          index={1000}
          componentProps = {{selectRow:SelectStoneType,formType:formType}}
          minHeight = '10rem'
          minWidth = '15rem'
      />
    </div>
...
export default Definition_Stone;
