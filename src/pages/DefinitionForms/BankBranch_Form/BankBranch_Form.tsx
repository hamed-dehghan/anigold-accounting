...
function BankBranch_Form({onSave,selectRow,formType}:any){
  ...
  useEffect(() => { 
    if(selectRow && formType=='Edit'){
      // apiService.Get_BankBranchGetById(selectRow?.id)
      // .then((response:any) => {
  ...
  function onSubmitProject(e:any){
    ...
    if(formType=='Edit'){
      ...
      apiService.Edit_Bankbranch(data)        
      .then((response:any) => {
      ...
      apiService.Create_Bankbranch(data)        
      .then((response:any) => {
        console.log(response?.data?.isSuccessful);
        if(response?.data?.isSuccessful){
          setOpenNotification({open:true,type:'success',message:`شعبه ${Modal_getValues('name')} با کد ${Modal_getValues('code')} تایید شد.`})
          onSave()        
          handleFocus('name')
  ...
  return(
    <div className="BankBranch_Form">
      <div className='react_hook_form'   >
        <form autoComplete="off" style={{display:'flex',justifyContent:'space-between',padding:'.5rem'}}>
            <div style={{display:'flex'}}>
              <div className='Field_Container'>
                <label>نام *</label>
                <input type='text'  {...Modal_register("name", { required: true })} placeholder={''} ref={(el) => {Modal_register("name").ref(el);(inputRefs.current['name'] = el)}}  onKeyDown={(e) => handleKeyDown(e, 'code')}/>   
                {Modal_errors.name && <div className='field_error'> نام را وارد کنید.</div>}
              </div>
              <div className='Field_Container'>
                <label>کد</label>
                <input type='number'  {...Modal_register("code", { required: false })} placeholder={''} ref={(el) => {Modal_register("code").ref(el);(inputRefs.current['code'] = el)}}  onKeyDown={(e) => handleKeyDown(e, 'Confirm')}/>   
...
export default BankBranch_Form;
