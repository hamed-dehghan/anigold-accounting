...
function Zone_Form({onSave,selectRow,formType}:any){
  ...
  useEffect(() => { 
    if(selectRow && formType=='Edit'){
      // apiService.Get_ZoneGetById(selectRow?.id)
      // .then((response:any) => {
  ...
  function onSubmitProject(e:any){
    ...
    if(formType=='Edit'){
      ...
      apiService.Create_Zone(data)        
      .then((response:any) => {
        console.log(response?.data?.isSuccessful);
        if(response?.data?.isSuccessful){
          setOpenNotification({open:true,type:'success',object:{messageType:'create',name:'منطقه',nameValue:Modal_getValues('name'),code:Modal_getValues('code')}})
          onSave()        
          handleFocus('name')
