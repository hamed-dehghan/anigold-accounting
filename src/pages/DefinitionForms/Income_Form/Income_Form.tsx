...
function Income_Form({onSave,selectRow,formType}:any){
  ...
  useEffect(() => { 
    if(selectRow && formType=='Edit'){
      // apiService.Get_IncomeGetById(selectRow?.id)
      // .then((response:any) => {
  ...
  function GeneratedCode(){
    // apiService.GeneratedCode_Income()
    // .then((response:any) => {
  ...
  function onSubmitProject(e:any){
    ...
    if(formType=='Edit'){
      ...
      apiService.Edit_Income(data)        
      .then((response:any) => {
        if(response?.data?.isSuccessful){
          setOpenNotification({open:true,type:'success',object:{messageType:'update',name:'درآمد',nameValue:Modal_getValues('name'),code:Modal_getValues('code')}})
          onSave({close:true})
      ...
      apiService.Create_Income(data)        
      .then((response:any) => {
        console.log(response?.data?.isSuccessful);
        if(response?.data?.isSuccessful){
          setOpenNotification({open:true,type:'success',object:{messageType:'create',name:'درآمد',nameValue:Modal_getValues('name'),code:Modal_getValues('code')}})
          onSave()        
          handleFocus('name')
