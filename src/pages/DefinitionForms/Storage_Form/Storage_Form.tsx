...
function Storage_Form({onSave,selectRow,formType}:any){
  ...
  const { control: Modal_control,handleSubmit:Modal_handleSubmit, register : Modal_register,reset:Modal_reset,getValues:Modal_getValues,setValue:Modal_setValue,formState: { errors : Modal_errors }} :any = useForm(
    {
      defaultValues: {
        census: false,
      } 
  ...
  useEffect(() => { 
    if(selectRow && formType=='Edit'){
      // apiService.Get_StorageGetById(selectRow?.id)
      // .then((response:any) => {
  ...
  function Set_AllData(selectRow:any){
    console.log(selectRow?.census);
    
    Modal_setValue('name',selectRow?.name)
    Modal_setValue('code',selectRow?.code)
    Modal_setValue('census',selectRow?.census)
    Modal_setValue('storageType',selectRow?.storageType)
  ...
  function GeneratedCode(){
    apiService.GeneratedCode_Storage()
    .then((response:any) => {
  ...
  function onSubmitProject(e:any){
    ...
    let data :any = {
      ...
      "storageType": Modal_getValues('storageType')?.id,
      "census": Modal_getValues('census'),
    }
    console.log(data);
    if(formType=='Edit'){
      data.id = selectRow?.id
      apiService.Edit_Storage(data)        
      .then((response:any) => {
        if(response?.data?.isSuccessful){
          setOpenNotification({open:true,type:'success',object:{messageType:'update',name:'صندوق',nameValue:Modal_getValues('name'),code:Modal_getValues('code')}})
          onSave({close:true})
      ...
      apiService.Create_Storage(data)        
      .then((response:any) => {
        console.log(response?.data?.isSuccessful);
        if(response?.data?.isSuccessful){
          setOpenNotification({open:true,type:'success',object:{messageType:'create',name:'صندوق',nameValue:Modal_getValues('name'),code:Modal_getValues('code')}})
          onSave()        
          handleFocus('name')
  ...
  return(
    <div className="Storage_Form">
      <div className='react_hook_form'   >
        <form autoComplete="off" style={{display:'flex',flexDirection:'column',padding:'.5rem'}}>
            ...
            <div style={{display:'flex'}}>
              <div className='Field_Container'>
                <label className="label">نوع صندوق</label>
                <Controller
                  name="storageType"
                ...
                />
                {Modal_errors.bankId && <div className='field_error'>نوع صندوق را وارد کنید.</div>}
              </div>
              <FormControlLabel {...Modal_register("census")}  control={<Checkbox  checked={Modal_getValues("census")} onChange={(e) => Modal_setValue("census", e.target.checked)}/>} label="سرشماری" style={{width:'25rem'}}/>
            </div>
        </form>
      </div>
